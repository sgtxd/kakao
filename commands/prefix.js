...
module.exports.run = async (bot, message, args, db, config) => {
    ...
    var guild_settings = await db.prepare(`SELECT * FROM guild_settings WHERE guild_id = ?;`).get(message.guild.id);
    let oldprefix = guild_settings.prefix
    await db.prepare(`UPDATE guild_settings SET prefix = '${args[0]}' WHERE guild_id = '${message.guild.id}'; `).run()
